<?php
	class Type{
		...
		public function getAllProductTypeForMenu()
		{
			$db = Database::getInstance()->getConnection();
			$query = $db->prepare("SELECT * FROM product_type ORDER BY type_id DESC");
			$query->execute();
			return $query->fetchAll(PDO::FETCH_ASSOC);
        }
		...
		public function getSingleTypeName($type_name)
		{
			$db = Database::getInstance()->getConnection();
            $query = $db->prepare("SELECT * FROM product_type WHERE type_name=:type_name");
            $query->bindValue(":type_name", $type_name);
		...
		public function getSingleId($type_id)
		{
			$db = Database::getInstance()->getConnection();
		...
		public function getTypeGenreNew($type_id)
		{
			$db = Database::getInstance()->getConnection();
            $query = $db->prepare("SELECT * FROM genre WHERE type_id=:type_id");
            $query->bindValue(":type_id", $type_id);
